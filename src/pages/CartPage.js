...
const CartPage = () => {
  ...
  // Nút xóa duy nhất (Smart Remove)
  const handleSmartRemove = () => {
    if (selectedItems.length > 0) {
      if (window.confirm(`Xóa ${selectedItems.length} sản phẩm đã chọn khỏi giỏ hàng?`)) {
        ...
        <div className="cart-content">
            ...
            <div className="cart-items-header">
              <div className="select-all-area">
                <input
                    ...
                    checked={selectedItems.length === cartDetails.length && cartDetails.length > 0}
                        ...
                        <div className="cart-item-controls">
                          <div className="control-left-group">
                            <div className="quantity-control">
                              <button className="quantity-btn" onClick={() => handleQuantityChange(item.productId, item.quantity - 1)} disabled={item.quantity <= 1 || updating === item.productId}>-</button>
                              <input type="number" value={item.quantity} readOnly className="quantity-input" />
                              <button className="quantity-btn" onClick={() => handleQuantityChange(item.productId, item.quantity + 1)} disabled={item.quantity >= item.product.stock || updating === item.productId}>+</button>
                            </div>
          ...
          <div className="cart-summary-section">
            <div className="cart-summary-card">
              ...
              <div className="summary-row"><span>Phí vận chuyển:</span><span className={shipping === 0 ? 'free-shipping' : ''} style={{ color: shipping === 0 ? 'var(--success)' : 'inherit' }}>{shipping === 0 ? 'Miễn phí' : formatPrice(shipping)}</span></div>
